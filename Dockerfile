...
# Add a health check to verify if the user service is running correctly
HEALTHCHECK --interval=30s --timeout=5s --start-period=30s --retries=3 CMD curl -f http://localhost:8081/actuator/health || exit 1
